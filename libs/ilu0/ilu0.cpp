...
static inline void gather_blocks(__m512d out[9], const double *vals, __m512i voff, __mmask8 m) {
...
static inline void scatter_blocks(double *vals, __m512i voff, const __m512d in[9], __mmask8 m)
{
    for (int e = 0; e < 9; e++) {
        __m512i idx = _mm512_add_epi64(voff, _mm512_set1_epi64(e));
...
static inline void process_tile(double *vals, const int64_t *off_ij, const int64_t *off_kj, int n) {
...
void ilu0_decomposition_avx512(BlockedCSR &A) {
    ...
    for (int i = 0; i < A.nb; i++) {
        ...
        for (int p = row_start; p < row_end; p++) {
            ...
            int64_t offset_ij[batch_size];
            int64_t offset_kj[batch_size];
            int counter = 0;
            ...
            for (int q = p + 1; q < row_end; q++) {
                ...
                offset_ij[counter] = (int64_t) q * bs2;
                offset_kj[counter] = block_kj - A.vals;

                counter++;

                if (counter == batch_size) {
                    process_tile(A.vals, offset_ij, offset_kj, counter);
                    counter = 0;
                }
            }
